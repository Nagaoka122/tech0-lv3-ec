...
flowchart LR
    ...
    BATCH["日次バッチ"] --> SVC
    API --> SVC["業務サービス層<br/>予定日の算出・在庫更新・注文"]
    SVC --> DB[("PostgreSQL")]
```
...
次回購入予定日の算出は業務サービス層に置き、日次バッチと画面表示の両方から同じ実装を使う。
...
一度も購入していなければ、セットに入れる品目を履歴から選べない。購入機能はセット機能の前提に
なっている。
...
### 2.2 アクティビティ図：セット注文の確定

本アプリで最も判断が詰まっている処理である。在庫・金額・割引券という3つの検証を通し、
1つでも通らなければ全体を取り消す。

```mermaid
flowchart TD
    START([注文の要求]) --> AUTH{"本人のセットか"}
    AUTH -->|いいえ| E404["404 対象が見つかりません"]
    AUTH -->|はい| QTY{"合計数量が20点以下か"}
    QTY -->|いいえ| E422["422 数量が上限を超えています"]
    QTY -->|はい| TX["トランザクション開始"]
    TX --> LOCK["対象SKUの在庫行を<br/>sku_id 昇順でロック"]
    LOCK --> STOCK{"すべての品目に<br/>在庫があるか"}
    STOCK -->|いいえ| RB1["ロールバック"]
    RB1 --> E409A["409 在庫が変わりました"]
    STOCK -->|はい| SUB["単価をDBから取得し<br/>小計を再計算"]
    SUB --> HASC{"割引券の指定が<br/>あるか"}
    HASC -->|いいえ| TOTAL["支払額 = 小計"]
    HASC -->|はい| CLOCK["券をロックし<br/>所有者・未使用・期限内を検証"]
    CLOCK --> CVALID{"券は有効か"}
    CVALID -->|いいえ| RB2["ロールバック"]
    RB2 --> E409B["409 この割引券は使用できません"]
    CVALID -->|はい| DISC["割引額 = min(券の金額, 小計)"]
    DISC --> TOTAL2["支払額 = 小計 - 割引額"]
    TOTAL --> CMP
    TOTAL2 --> CMP{"支払額が画面の<br/>表示額と一致するか"}
    CMP -->|いいえ| RB3["ロールバック"]
    RB3 --> E409C["409 価格が変更されています"]
    CMP -->|はい| PAY{"模擬決済に<br/>成功したか"}
    PAY -->|いいえ| RB4["ロールバック"]
    RB4 --> E402["402 決済に失敗しました"]
    PAY -->|はい| COMMIT["在庫を減算<br/>注文と明細を作成<br/>券を使用済みに<br/>リマインドを完了に"]
    COMMIT --> DONE([コミット / 201])
```

#### 設計判断1　検証をすべてトランザクションの内側に置く

在庫・金額・券の検証を「先に確かめてから書き込む」二段構えにすると、確認と書き込みの間に
別の注文が割り込める。**判定に使った値をロックしたまま書き込みまで進める**ことで、確認した
状態のまま確定することを保証する。

#### 設計判断2　在庫行を `sku_id` の昇順でロックする

複数の注文が異なる順序でロックを取ると、互いの解放を待ち合ってデッドロックになる。
**ロック順序を全処理で統一する**ことで、待ち合いの循環そのものを作らない。

#### 設計判断3　割引額を `min(券の金額, 小計)` で求める

券の金額が小計を上回っても支払額が負にならないことを、**計算式の段階で保証する**。
「マイナスになったら0に丸める」という後段の補正に頼ると、補正を通らない経路から不正な金額が
成立しうる。差額の繰り越しは扱わない（N-08）。

#### 設計判断4　部分的な成立を認めない

1品目でも在庫が足りなければ注文全体を取り消す。一部だけ買えた状態を作ると、残りをどう扱うか
（取り置き、追加注文、通知）という別の設計が必要になる。まとめ買いを定番セット単位に限った
のは、この単純な規則で済ませるためである。

### 2.3 アクティビティ図：日次バッチ

```mermaid
flowchart TD
    B0([日次バッチ開始]) --> B1{"未処理のセットが<br/>あるか"}
    B1 -->|なし| B9["有効期限を過ぎた<br/>未使用の割引券を失効させる"]
    B9 --> BFIN([終了])
    B1 -->|あり| B2{"買い替え月が<br/>登録されているか"}
    B2 -->|いいえ| B1
    B2 -->|はい| B3["候補日 = 本年の買い替え月の1日"]
    B3 --> B4{"候補日は<br/>本日より前か"}
    B4 -->|はい| B5["候補日 = 翌年の買い替え月の1日"]
    B4 -->|いいえ| B6
    B5 --> B6{"現在の予定日が<br/>候補日より後か<br/>（見送り済み）"}
    B6 -->|はい| B1
    B6 -->|いいえ| B7["次回購入予定日を<br/>候補日で更新"]
    B7 --> B8{"予定日の14日前を過ぎ<br/>かつリマインド未作成か"}
    B8 -->|いいえ| B1
    B8 -->|はい| BM["リマインドを作成<br/>対象の予定日を記録"]
    BM --> B1
```

**購入履歴から周期を計算しない。** 対象が季節性のある衣類であり、買い替えは1年周期で決まって
いる。履歴は買い替え月の候補を提示するためだけに使う。

当初は購入間隔の中央値から周期を推定する設計だったが、次の理由で採らなかった。

1. 周期は実質的に1年で決まっており、推定するまでもない。
2. 1回も買っていない状態では推定できず、**利用開始の初年度に機能しない**。
3. 間隔のばらつきで推定の可否を判定すると、**購入回数が多い利用者ほど「ばらつきがある」と
   判定されて機能が止まる**という逆転が起きる。

見送り済みの予定日を引き戻さない判定（B6）を入れているのは、翌日のバッチが「本年の買い替え
月」を再計算して、見送りを打ち消してしまわないようにするためである。
...
classDiagram
    ...
    class PurchaseSchedule {
        +nextDateFor(StapleSet, date) date
        +suggestMonth(Member, StapleSet) int
    }
    ...
    PurchaseSchedule ..> OrderLine : 候補月を参照
    PurchaseSchedule ..> StapleSet : 予定日を更新
...
`PurchaseSchedule.nextDateFor()` は日次バッチと画面表示の両方から使う。買い替え月から
予定日を導くだけの純粋な計算であり、状態を持たない。`suggestMonth()` は購入履歴の直近の
購入月を返すもので、登録画面の初期値にのみ使う。**推定ではないため、外れても機能が止まらない。**

`OrderService` は単品購入（`placeSingle`）とセット注文（`placeSet`）で在庫減算のロジックを
共有する。

`TradeInItem.usedDays()` は購入日と返送日の差を返す。今回この値は使わないが、返送を
購入履歴と結びつけて記録しているために算出できる。将来、購入間隔ではなく実使用期間から
買い替え時期を見直す場合（S5）の入力になる。
...
| 項目 | 設計 | 理由 |
|---|---|---|
| `ORDER_LINE.sku_id` | 品番ではなくSKUを参照する | 「去年と同じもの」は色・サイズまで一致して初めて成立する。後からSKU化しても過去の色・サイズは復元できない |
| `ORDERS.ordered_at` | 明細から注文日をたどれるようにする | 買い替え月の候補提示と、下取りの実使用期間の算出に使う。集計値だけを保存すると再現できない |
| `STAPLE_SET.purchase_month` | 1〜12。NULL は未登録 | **本人が決めた設定であり、履歴から再現できない。** NULL のセットは予定日を持たず、通知の対象にしない |
| `STAPLE_SET.next_purchase_date` | 買い替え月から算出した値を保持 | 見送りで翌年へ進めた状態を保持するために、算出値をそのまま持つ。持たないと見送りを表現できない |
| `STAPLE_SET_ITEM` | SKUと数量だけを持つ | 買い替え月はセットが持つ。**まとめ買いはセット全体を同じ時期に買うことなので、品目ごとに時期を持つ意味がない** |
...
| 日付 | 予定日・購入日・返送日・券の期限は `date` 型 | 予定日と期限の計算は日単位で行う。時刻の差で結果が変わらないようにする |
...
```typescript
type ScheduleState = "CONFIGURED" | "UNSET";
```
...
type StapleSetSummary = {
  ...
  purchase_month: number | null;         // integer 1〜12。未登録は null
...
type StapleSetItemDetail = {
  ...
  last_purchased_on: string | null;      // YYYY-MM-DD 参考表示
};
type StapleSetDetailResponse = {
  set_id: string;
  name: string;
  purchase_month: number | null;         // integer 1〜12
  next_purchase_date: string | null;
  schedule_state: ScheduleState;
  suggested_purchase_month: number | null; // 履歴の直近購入月。未登録時の初期値候補
  items: StapleSetItemDetail[];
};
```

`suggested_purchase_month` は購入履歴の直近の購入月である。**設定値ではなく提案であり、
登録画面の初期値としてのみ使う。** 購入者が登録するまで `purchase_month` は `null` のまま
であり、通知も行われない。
...
type UpdateStapleSetRequest = {
  name?: string;                          // 1〜50文字
  purchase_month?: number | null;         // 1〜12 で登録、null で解除
};
```

`purchase_month` に1〜12を渡すと登録され、次回購入予定日がその月の1日（過ぎていれば翌年の
同月1日）として算出される。`null` を渡すと解除され、予定日を持たなくなり通知も止まる。

**買い替え月は本人が変更するまで維持される。** 購入履歴が増えてもバッチが書き換えることは
ない。設定と推定が競合する経路そのものを作らない。
...
// skip の応答：次回購入予定日が翌年の同月1日へ進む
type SkipResponse = { next_purchase_date: string | null };
```

買い替え月を登録していないセットにはリマインドが作られないため、`skip` の対象にならない。
呼ばれた場合は404を返す。
...
| purchase_month | integer | 1 | 12 | — | 422 |
...
買い替え月を1〜12に限るのは自明だが、サーバ側でも検証する。画面が月の選択肢しか出さなくても、
範囲外の値を直接送られる経路があるためである（S-05）。
...
| 事象 | 処理 |
|---|---|
| バッチが途中で失敗 | 当日分を再実行できる。予定日は買い替え月からの再計算であり、リマインドは一意制約で重複しない |
| バッチが当日実行されなかった | 翌日の実行で追いつく。リマインドは「予定日の14日前を過ぎ、かつ未作成」で判定するため、1日遅れても作成される |
| 処理中に例外が発生したセットがある | そのセットのみ記録して次へ進み、他のセットの処理を止めない |
| 見送りで先へ進めた予定日 | 候補日より後の予定日は更新しない。バッチが見送りを打ち消さない |
...
stateDiagram-v2
    [*] --> UNSET : セット作成直後
    UNSET --> CONFIGURED : 本人が買い替え月を登録
    CONFIGURED --> CONFIGURED : 本人が月を変更（予定日を再算出）
    CONFIGURED --> UNSET : 本人が解除（予定日を持たず通知しない）
```

`UNSET` の間は次回購入予定日を持たず、リマインドも作られない。**買い替え月を書き換えるのは
本人だけであり、バッチは書き換えない。** 設定と自動処理が競合する経路そのものを作らない
設計としている。

次回購入予定日そのものは、`CONFIGURED` のセットについてバッチが更新する（2.3）。予定日が
過ぎれば翌年の同月1日へ進み、見送られていればそれを尊重する。
...
| 対象 | 確認内容 |
|---|---|
| 予定日の算出 | 買い替え月に4を登録すると、次回購入予定日が本年4月1日になる |
| 年またぎ | 本日が5月で買い替え月が4のとき、予定日が翌年4月1日になる |
| 境界 | 本日が4月1日のとき、予定日は本年4月1日のまま（過ぎていない） |
| リマインド | 予定日の15日前では作成されず、14日前で作成される |
| 未登録 | 買い替え月が未登録のセットには予定日もリマインドも作られない |
| 解除 | 登録済みのセットを解除すると、予定日が消えリマインドが止まる |
| 範囲 | 買い替え月に0や13を送ると422で拒否される |

### 9.2 設定の保護と見送り

| 対象 | 確認内容 |
|---|---|
| 上書き防止 | 買い替え月を登録したあとに購入が増えても、バッチ実行後に月が変わらない |
| 候補提示 | 直近の購入月が候補として返るが、登録するまで `purchase_month` は null のまま |
| 履歴ゼロ | 購入履歴が0件のセットでも買い替え月を登録でき、予定日と通知が働く |
| 見送り | 見送ると予定日が翌年の同月1日へ進む |
| 引き戻しの防止 | 見送った翌日にバッチを実行しても、予定日が本年へ戻らない |
| 再通知 | 見送った予定日に対してリマインドが再作成されない |
...
| 例外の隔離 | 1つのセットで例外が起きても、他のセットの処理が完了する |
| 券の失効 | 同日に2回実行しても、失効した券の件数と状態が変わらない |

### 9.7 検証用データ

- 会員：複数会員を用意し、他人のセットを参照できないことを確認できるようにする。
- 購入履歴：直近の購入月が異なる複数のSKU（候補提示の確認用）、3年ちょうど前と3年と1日前の
  購入（遡及期間の境界）、購入履歴が0件の会員。
- 在庫：0のSKUと1のSKUを含め、確認画面での除外と同時注文を再現できるようにする。
- セット：買い替え月が登録済みのもの、未登録のもの、見送り済みのもの、品目が0件のものを各1件以上。買い替え月は本日より前の月・後の月の両方を用意する。
...
|---|---|---|---|
| 1 | 商品管理を品番単位からSKU単位へ変更する | 色・サイズを区別しなければ「去年と同じもの」を履歴から選べない | 仕組みは後から追加できる |
| 2 | 取引明細にSKUと取引日を保存する | 店頭購入を履歴に並べ、買い替え月の候補と下取りの対象にするため | **取り戻せない。** 記録前の取引の色・サイズと日付は復元できない |
| 3 | 取引に会員IDを保存する | 店頭購入を誰の履歴に含めるか判別するため | **取り戻せない。** 記録前の取引が誰のものか判別できない |
...
| D-04 | 買う時期のずれの想定幅 | ±45日は実測にもとづかない。割引券の有効期限に直結するため、運用後に見直す |
| D-05 | 品番が変わった定番商品の扱い | セットに登録したSKUが終売になった場合の扱い。世代の紐づけは将来スコープS4 |
