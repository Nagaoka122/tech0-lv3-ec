...
sequenceDiagram
    ...
    B->>F: POST /api/me/staple-sets/{set_id}/orders {lines, expected_total_yen}
    ...
    alt いずれかの在庫が不足
        ...
        A->>A: 単価をDBから取得し合計を再計算
        alt 合計が expected_total_yen と不一致
            A->>D: ROLLBACK
            A-->>F: 409 価格が変更されています
        else 一致
            A->>A: 模擬決済を実行
...
classDiagram
    ...
    class OrderService {
        ...
        +placeSet(Member, StapleSet, Line[], int) Order
    }
...
| 日付 | 予定日・購入日は `date` 型 | 周期の計算は日単位で行う。時刻の差で結果が変わらないようにする |
...
一方、**セット・品目・リマインドは識別子をパスに含める。** これらは他人のIDを指定できる
経路になるため、`resource.member_id == token.sub` の検証を必須とする（8.2）。
...
type CreateSetOrderRequest = {
  ...
  expected_total_yen: number;        // integer 画面に表示していた合計
};
...
| 409 | 会員登録の重複、品目数の上限超過、在庫不足、価格変更 | 状況に応じた説明を表示 | 残す：set_id、sku_id、判定結果 |
| 422 | 型・範囲の検証エラー（数量、合計点数、日付、文字列長） | 項目ごとの説明を表示 | 残す：項目名と違反種別／残さない：値そのもの |
...
| 入力は妥当だが、現在の状態と衝突する | 在庫不足、品目が既に10件、価格変更 | 409 |
...
| 所有者検証 | `set_id` `item_id` `reminder_id` をパスに含むエンドポイントでは、必ず `resource.member_id == token.sub` を検証する |
...
| 対象 | 設計 |
|---|---|
| 金額 | クライアントから受け取った `expected_total_yen` を保存や請求に使わない。単価はDBから取得して合計を再計算し、一致しない場合は409で拒否する |
...
から確定するまでの間に在庫や価格が変わりうるため、確定時にすべて取り直す。
...
- ログにトークン、パスワード、購入履歴や定番セットの内容を出力しない。
